...
figure(2)
subplot(3,1,1)
plot(out_T.tout, squeeze(out_T.w_b_LVLH.signals.values))
title('Angular Velocities')
ylabel('angular velocity (rad/sec)')
legend('\omega_x','\omega_y','\omega_z')
grid on
...
subplot(3,1,2)
plot(out_T.tout, squeeze(out_T.q_b_LVLH.signals.values))
title('Quaternions')
ylabel('Quaternion Parameter')
legend('q_1','q_2','q_3','q_4')
grid on
...
subplot(3,1,3)
plot(out_T.tout, squeeze(out_T.E_b_LVLH.signals.values))
title('Euler Angles')
xlabel('time (seconds)')
ylabel('Angle (rad)')
legend('\phi','\theta','\psi')
